...
const StyledTableCell = styled(TableCell)<{ isSmall?: boolean }>(
  ({ theme, isSmall }) => ({
...
export default function LightTable({
  ...
  const toSortFn = (column: ColumnsProps) => {
    ...
    if (sortKey !== column.key) {
      ...
      const sortV =
        sortValue === "" ? "ascend" : sortValue === "ascend" ? "desc" : "";
      sortVal = sortV;
      setSortValue(sortV);
      if (sortV === "") {
        setSortKey("");
    ...
    <>
      {
        <TableContainer style={{ boxShadow: "none", minWidth: "650px" }}>
                      ...
                      {column.sorter ? (
                        <Box sx={{ display: "flex", flexDirection: "column" }}>
                          <SortBox
                            icon={`up`}
                            active={
                              sortKey === column.key && sortValue === "ascend"
                            }
                          ></SortBox>
                          <SortBox
                            icon={`down`}
                            active={
                              sortKey === column.key && sortValue === "desc"
                            }
                          ></SortBox>
                        </Box>
                      ) : undefined}
          ...
          {tableData <= 0 && (
            <NoData component="div">{emptyText ? emptyText : "No Data"}</NoData>
          )}
        </TableContainer>
      }
    </>
